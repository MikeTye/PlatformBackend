...
DROP TABLE IF EXISTS project_registry_links CASCADE;
...
CREATE TABLE projects (
  ...
  -- New: high-level classification & location (good for filtering/search)
  sector                      text,                                   -- e.g. Energy, Forestry, Waste...
  host_country                text,                                   -- ISO country or free-text
  host_region                 text,                                   -- District / region
  ...
  registry_date               date,                                   -- generic registry date

  -- New: more explicit dates as per RFP project forms
  ...
  methodology_version         text,                                   -- e.g. "v1.0"
  ...
  -- New: RFP-style quantitative projections
  expected_annual_reductions  jsonb DEFAULT '{}'::jsonb,             -- { "2026": "10000", ... } tCO2e
  volume_offered_authority    numeric(20,6),                          -- potential volume to offer (tCO2e)
  tenderer_role               text,                                   -- developer / investor / trader, etc.
...
CREATE INDEX projects_sector_idx
  ON projects (sector);

CREATE INDEX projects_host_country_idx
  ON projects (host_country);
