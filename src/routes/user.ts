...
router.get("/", async (req, res) => {
    try {
        ...
        SELECT
            ...
            COALESCE(cc.company_count, 0)::int AS company_count,
        ...
        LEFT JOIN LATERAL (
            ...
            FROM companies c2
            WHERE c2.owner_user_id = up.user_id
            AND c2.delete_flag = false
        ) cc ON true

        LEFT JOIN LATERAL (
            SELECT COUNT(*)::int AS project_count
            FROM projects p2
            WHERE p2.owner_user_id = up.user_id
            AND p2.delete_flag = false
        ) pc ON true
